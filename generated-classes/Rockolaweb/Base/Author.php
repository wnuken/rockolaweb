...
use Rockolaweb\Author as ChildAuthor;
use Rockolaweb\AuthorQuery as ChildAuthorQuery;
use Rockolaweb\Music as ChildMusic;
use Rockolaweb\MusicQuery as ChildMusicQuery;
use Rockolaweb\Map\AuthorTableMap;
...
abstract class Author implements ActiveRecordInterface
{
     ...
     * TableMap class name
     */
    const TABLE_MAP = '\\Rockolaweb\\Map\\AuthorTableMap';
    ...
    /**
     * Initializes internal state of Rockolaweb\Base\Author object.
     */
     ...
     * @return $this|\Rockolaweb\Author The current object (for fluent API support)
     */
    ...
    public function setName($v)
    {
        ...
        if ($this->name !== $v) {
            $this->name = $v;
            $this->modifiedColumns[AuthorTableMap::COL_NAME] = true;
        }
    ...
    public function setNationality($v)
    {
        ...
        if ($this->nationality !== $v) {
            $this->nationality = $v;
            $this->modifiedColumns[AuthorTableMap::COL_NATIONALITY] = true;
        }
    ...
    public function hydrate($row, $startcol = 0, $rehydrate = false, $indexType = TableMap::TYPE_NUM)
    {
        try {
            ...
            $col = $row[TableMap::TYPE_NUM == $indexType ? 1 + $startcol : AuthorTableMap::translateFieldName('Name', TableMap::TYPE_PHPNAME, $indexType)];
            $this->name = (null !== $col) ? (string) $col : null;

            $col = $row[TableMap::TYPE_NUM == $indexType ? 2 + $startcol : AuthorTableMap::translateFieldName('Nationality', TableMap::TYPE_PHPNAME, $indexType)];
            ...
            throw new PropelException(sprintf('Error populating %s object', '\\Rockolaweb\\Author'), 0, $e);
        }
    }
    ...
    protected function doInsert(ConnectionInterface $con)
    {
        ...
        if ($this->isColumnModified(AuthorTableMap::COL_NAME)) {
            $modifiedColumns[':p' . $index++]  = 'name';
        }
        if ($this->isColumnModified(AuthorTableMap::COL_NATIONALITY)) {
    ...
    public function fromArray($arr, $keyType = TableMap::TYPE_PHPNAME)
    {
        ...
        if (array_key_exists($keys[2], $arr)) {
            $this->setNationality($arr[$keys[2]]);
        }
    }
     ...
     * @param string $keyType The type of keys the array uses.
     *
     * @return $this|\Rockolaweb\Author The current object, for fluid interface
     */
    ...
    public function buildCriteria()
    {
        ...
        if ($this->isColumnModified(AuthorTableMap::COL_NAME)) {
            $criteria->add(AuthorTableMap::COL_NAME, $this->name);
        }
        if ($this->isColumnModified(AuthorTableMap::COL_NATIONALITY)) {
            $criteria->add(AuthorTableMap::COL_NATIONALITY, $this->nationality);
        }
     ...
     * objects.
     *
     * @param      object $copyObj An object of \Rockolaweb\Author (or compatible) type.
     ...
     *
     * @param  boolean $deepCopy Whether to also copy all rows that refer (by fkey) to the current row.
     * @return \Rockolaweb\Author Clone of current object.
     * @throws PropelException
     */
     ...
     * @return ObjectCollection|ChildMusic[] List of ChildMusic objects
     */
    public function getMusicsJoinGenders(Criteria $criteria = null, ConnectionInterface $con = null, $joinBehavior = Criteria::LEFT_JOIN)
    {
        $query = ChildMusicQuery::create(null, $criteria);
        $query->joinWith('Genders', $joinBehavior);
